...
import com.kayo.drag.listener.DragListener;
import com.kayo.drag.listener.DragListenerII;
import com.kayo.drag.util.ViewDragHelper;
...
public class DragLayout extends FrameLayout {
    ...
    public DragLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.DragLayout, defStyle,
                R.style.SwipeBackLayout);

        int edgeSize = a.getDimensionPixelSize(R.styleable.DragLayout_edge_size, -1);
        if (edgeSize > 0)
            setEdgeSize(edgeSize);
        int mode = EDGE_FLAGS[a.getInt(R.styleable.DragLayout_edge_flag, 0)];
        setEdgeTrackingEnabled(mode);

        int shadowLeft = a.getResourceId(R.styleable.DragLayout_shadow_left,
                R.drawable.shape_shadow_left);
        int shadowRight = a.getResourceId(R.styleable.DragLayout_shadow_right,
                R.drawable.shape_shadow_right);
        int shadowBottom = a.getResourceId(R.styleable.DragLayout_shadow_bottom,
                R.drawable.shape_shadow_bottom);
     ...
     * @deprecated use {@link #bindDragListener} instead
     */
    @Deprecated
    public void setDragListener(DragListener listener) {
        bindDragListener(listener);
    }

    public void bindDragListener(DragListener listener) {
        if (mListeners == null) {
    ...
    public void scrollToFinishActivity() {
        ...
        if ((mEdgeFlag & EDGE_LEFT) != 0) {
            ...
            top = -childHeight - mShadowBottom.getIntrinsicHeight() - OVER_SCROLL_DISTANCE;
    ...
    public void attachToActivity(FragmentActivity activity) {
        ...
        bindDragListener(new DragListenerAdapter(activity));
        decor.addView(this);
    }
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            if (mScrollPercent >= 1) {
                if (null != mListeners && !mListeners.isEmpty()) {
                    for (DragListener listener : mListeners) {
                        if (listener instanceof DragListenerII) {
                            ((DragListenerII) listener).onContentViewDragLimit();
                        }
                    }
                }
            }
