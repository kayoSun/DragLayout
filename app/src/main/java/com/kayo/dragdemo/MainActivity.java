...
import com.kayo.drag.DragActivity;
import com.kayo.drag.DragLayout;
import com.kayo.drag.listener.DragListener;

public class MainActivity extends DragActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTrackingModeGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                ...
                switch (checkedId) {
                    case R.id.mode_left:
                        edgeFlag = DragLayout.EDGE_LEFT;
                        break;
                    case R.id.mode_right:
                        edgeFlag = DragLayout.EDGE_RIGHT;
                        break;
                    case R.id.mode_bottom:
                        edgeFlag = DragLayout.EDGE_BOTTOM;
                        break;
                    default:
                        edgeFlag = DragLayout.EDGE_ALL;
                }
                mDragLayout.setEdgeTrackingEnabled(edgeFlag);
                saveTrackingMode(edgeFlag);
            }
        });
        mDragLayout.bindDragListener(new DragListener() {
            @Override
    ...
    private void restoreTrackingMode() {
        int flag = PreferenceUtils.getPrefInt(getApplicationContext(), mKeyTrackingMode,
                DragLayout.EDGE_LEFT);
        mDragLayout.setEdgeTrackingEnabled(flag);
        switch (flag) {
            case DragLayout.EDGE_LEFT:
                mTrackingModeGroup.check(R.id.mode_left);
                break;
            case DragLayout.EDGE_RIGHT:
                mTrackingModeGroup.check(R.id.mode_right);
                break;
            case DragLayout.EDGE_BOTTOM:
                mTrackingModeGroup.check(R.id.mode_bottom);
                break;
            case DragLayout.EDGE_ALL:
